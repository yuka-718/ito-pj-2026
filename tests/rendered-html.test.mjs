...
test("connects to the dynamic Codex and Oriedita API and makes input errors visible", async () => {
  ...
  assert.match(page, /操作AIと評価AIを分離/);
  ...
  assert.match(server, /codexReviewTrigger = 0/);
  assert.match(server, /independent_codex_rubric_v1/);
  assert.match(server, /runIndependentVisualEvaluation/);
